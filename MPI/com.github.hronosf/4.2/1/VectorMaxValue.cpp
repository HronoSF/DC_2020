...
int main(int argc, char *argv[]) {
    ...
    int *search_array = new int[array_len];  // The vector to search max
    ...
    if (my_rank == 0) {
        ...
        if (isSequential) {
            ...
            stop = MPI_Wtime();
            printf("The sequential search executed for %f second\n", stop - start);
        }
        ...
        stop = MPI_Wtime();
        printf("The parallel search executed for %f second\n", stop - start);
    } else {
